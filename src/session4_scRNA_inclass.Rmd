...
highlight_cluster <- function(selected_tage, selected_color){
  dittoDimPlot(Tcells, var = "Tage", size=1, reduction.use = "umap", cells.use = Tcells$Tage == selected_tage, colors = selected_color, legend.show = FALSE, ylab = NULL,  xlab = NULL, main = NULL, show.axes.numbers = FALSE)
}
```
...
for (x in 1:6){
single_plot<-highlight_cluster(selected_tage=tage[x], selected_color = cols[x])
plot_list[[x]] <- single_plot
              }
  
plot_list[[7]]<- main_umap  

#create object with all object plots
...
legend = dittoSeq:::.grab_legend(
  dittoDimPlot(Tcells, "Tage", reduction.use = 'umap',
               color.panel = dittoColors()[c(cols)])
  )
legend
```
