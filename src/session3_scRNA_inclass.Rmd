...
In case of published data, use the values they used.kick out dead cells and empty droplets.
...
```{r Inspect all metadata colnames to prevent accidental renaming}
...
```{r add age to meta}
# grep is cntrlF and replace analogue in R
#might work better in small case (why?)
#double quotes and single quotes can be used interchangably
...
# Add Tcelltype
#asking samples per lane-complex multiplexing that Dan did
...
#Use the help tab to understand what the default is for that function
...
#Use ? to look at help tab of function
...
```
```{r Looking at count and normalised data}
#Display count data
Tcells@assays$RNA@counts
...
Tcells <- FindVariableFeatures(object = Tcells,
                               ...
                               nfeatures = 2000 )
#2000 is common feature -50 or 60 cell types then they use 3000.
...
#if you run with ?help function-cntrlf and find ? and rerun everything
Tcells <- ScaleData(
  object = Tcells,
...
DimPlot(Tcells, 
        dims=c(1,2), 
        reduction='pca')
#arbitary units - can't compare units of PC
```
...
```
Higher PC values - lower variance captured-no longer a quadrant; this is one way to assess the data - looking and seeing reasonable separation.
